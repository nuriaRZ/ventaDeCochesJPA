package modelo.ventaDeCoches.controladores;
...
public class ClienteControlador extends Controlador {
	...
	public boolean exists(Cliente coche) {
		...
		boolean ok = true;
		try {
			Query q = em.createNativeQuery("SELECT * FROM tutorialjavacoches.cliente where id = ?", Cliente.class);
	...
	public static String toString (Cliente cliente) {
		return cliente.getDniNie()+ " " + cliente.getNombre() + " " + cliente.getNombre() + cliente.getApellidos();
	}
